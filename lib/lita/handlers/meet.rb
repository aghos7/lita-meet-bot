  ...
  module Handlers
    class Meet < Handler
      ...
      def start_standup(response)
        ...
        # update_room is borked, it needs a room; the response doesn't have one
      ...
      def started_at
        @started_at ||= redis.get('last_standup_started_at').present? ? Time.parse(redis.get('last_standup_started_at')) : nil
      end
      ...
      def can_respond?
        return started_at.present? && (Time.now - started_at) <= config.time_to_respond
      end
